...
from chatapp.ArkChatFramework.DialogManager.ibis_rules import *
from chatapp.ArkChatFramework.DialogManager.ibis_types import *
from chatapp.ArkChatFramework.DialogManager.trindikit import *
...
class Grammar(object):
    ...
    def interpret(self, user_input):
        ...
        try: return Ask(Question(user_input))
        except: pass
        try: return Answer(Ans(user_input))
        except: pass
        return None
...
class SimpleGenGrammar(Grammar):
    def __init__(self):
        ...
        self.addForm("icm:neg*sem", '이해 하지 못했습니다. ')
...
#         print("Domain.add_plan(trigger?��?��[{}]".format(trigger))
#         print("Domain.add_plan(plan?��?��[{}]".format(plan))
...
#         print("Domain.relevant--> ?��?��  answer[{}], question[{}]".format(answer,question))
        assert isinstance(answer, (ShortAns, Prop)), \
            "The answer %s must be a ShortAns, Prop" % answer
        assert isinstance(question, Question), \
            "The question %s must be a Question" % question
    ...
    def combine(self, question, answer):
        ...
        """
#         print("Domain.combine.question[{}]".format(question))
#         print("Domain.combine.answer[{}]".format(answer))
...
#         print("Domain.get_plan.question[{}]".format(question))
#         print("Domain.get_plan.question?�� Type[{}]".format(type(question)))
#         print("Domain.get_plan.self.plans?�� Type[{}]".format(type(self.plans)))
#         print("Domain.get_plan.self.plans.keys()[{}]".format(type(self.plans.keys())))
            
        for key in self.plans:
#             print("self.plans?�� key�?[{}],type[{}]".format(key, type(key)))
#             print("question�?[{}],type[{}]".format(question, type(question)))
#             if str(key) == str(question):
            if key == question:
                for construct in reversed(self.plans[key]):
                    planstack.push(construct)
 
        return planstack
#         question_str = str(question)         
#         for construct in reversed(self.plans.get(question_str, None)):
#             planstack.push(construct)
#         return planstack
...
class IBIS1(IBIS):
    ...
    def update(self):
        ...
        maybe(self.update_downdate_qud)
    ...
    update_downdate_qud = rule_group(downdate_qud)
