...
class ChattingiShiftHomepage(ChatClient):
    '''
    classdocs
    '''
    ...
    def __init__(self, work_dir):
        ...
        self.intents_file_name = os.path.join(work_dir, "ArkChatFramework/ArkNLU/DialogIntents/intents_home_ishift_kr.json")
        # logger.info("intents       파일 디렉토리[%s]" % input_file_name)
        self.input_training_data_file_name = os.path.join(work_dir, "ArkChatFramework/ArkNLU/NLUModel/ishift/training_data_home_kr")
        # logger.info("training      파일 디렉토리[%s]" % input_training_data_file_name)
        self.tflearn_logs_dir = os.path.join(work_dir, 'ArkChatFramework/ArkNLU/NLUModel/ishift/home_tflearn_kr_logs')
        # logger.info("tflearn_logs     디렉토리[%s]" % tflearn_logs_dir)
        self.tflearn_model_file_name = os.path.join(work_dir, 'ArkChatFramework/ArkNLU/NLUModel/ishift/model_home_kr.tflearn')
...
def home_NLULearning():

    print("현재 프로세스의 작업 디렉토리 [%s]" % os.getcwd())
    print("부모 디렉토리로 변경[%s]" % os.chdir(os.pardir))
    print("변경후 현재 프로세스의 작업 디렉토리 [%s]" % os.getcwd())
 
    print("intents 파일 디렉토리[%s]" % os.path.abspath("ArkNLU/DialogIntents/intents_home_ishift_kr.json"))

    input_file_name = os.path.abspath("ArkNLU/DialogIntents/intents_home_ishift_kr.json")
    print("intents       파일 디렉토리[%s]" % input_file_name)
    
    input_training_data_file_name = os.path.abspath("ArkNLU/NLUModel/ishift/training_data_home_kr")
    print("training      파일 디렉토리[%s]" % input_training_data_file_name)
    
    tflearn_logs_dir = os.path.abspath('ArkNLU/NLUModel/ishift/home_tflearn_kr_logs')
    print("tflearn_logs     디렉토리[%s]" % tflearn_logs_dir)
    
    tflearn_model_file_name = os.path.abspath('ArkNLU/NLUModel/ishift/model_home_kr.tflearn')
...
#     intents_file, training_data_file, tflearn_logs_dir, tflearn_model_file
    ...
    print("iShift home NLULearning instance...")
    
    if bot.create_learning_model(show_details=True):
        print("iShift home model creation success....")
    else:
        print("iShift home model creation fail....")
